  ...
  return (
    <main id="top">
      ...
      <section className="services section" id="szolgaltatasok"><div className="shell">
        ...
        <div className="services__grid">{services.map((service) => <article className={`service-card${service.icon === "wedding" ? " service-card--wedding" : ""}`} key={service.name}><ServiceIcon name={service.icon} /><h3>{service.name}</h3><strong className="service-card__title">{service.title}</strong><p>{service.text}</p><Link className="button service-card__button" href={service.href}>{service.cta}</Link></article>)}</div><p className="services__note">Az egyes szolgáltatások részletes, önálló céloldalai a következő fejlesztési ütemben készülnek el.</p>
      </div></section>

      <section className="booking-callout"><div className="shell booking-callout__inner"><div><p className="section-label">Asztalfoglalás</p><h2>Foglaljon asztalt online</h2><p>Egy közös ebéd, egy különleges vacsora vagy egy nagyobb családi alkalom előtt érdemes időben lefoglalni az asztalt.</p></div><div className="booking-callout__actions"><Link className="button button--brown" href="/asztalfoglalas">Asztalfoglalás</Link><a className="phone-link" href="tel:+36302644081">+36 30 264 4081</a></div></div></section>
